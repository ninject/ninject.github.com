...
Syntax.trimSpaces = function (text) {
	var offsets = [], totalOffset = 0;
    var length = 1000000;
	
	text = text.replace(/^[ ]*(?=\S)/gm, function(match) {
	        var currentLength = match.length;
			if (currentLength < length) length = match.length;
			return match;
		}
	);
	...
	if (length > 0) {
	    var regex = "^[ ]{" + length +"}"; 
		text = text.replace(new RegExp(regex, "gm"), function(match) {
			var offset = arguments[arguments.length - 2];

			totalOffset += length
			offsets.push([offset, length, totalOffset]);
	...
	elements.each(function () {
		...
		Syntax.brushes.get(brushName, function(brush) {
			if (options.tabWidth) {
			    ...
			    replacement = Syntax.trimSpaces(text);
				if (matches && matches.length) {
					var linearOffsets = Syntax.convertToLinearOffsets(replacement.offsets, text.length);
					matches = Syntax.updateMatchesWithOffsets(matches, linearOffsets, replacement.text);
				}
